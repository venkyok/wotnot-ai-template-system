...
async def get_active_conversations(
    ...
    phone_id = getattr(current_user, "Phone_id", None)
    if not phone_id:
        logger.warning(
            "Active conversation stream requested but user '%s' has no Phone_id configured",
    ...
    async def get_active_chats() -> AsyncGenerator[str, None]:
        ...
        while True:
            ...
            result = await db.execute(
                select(ChatBox.Last_Conversation)
                .filter(cast(ChatBox.Last_Conversation.receiver_wa_id, String) == str(phone_id))
