...
# Ensure environment variables are loaded when this module imports
load_dotenv()
...
async def verify_turnstile_token(token: str, remoteip: str = None) -> dict:
    if os.getenv("TURNSTILE_BYPASS", "false").lower() == "true":
        return {"success": True, "skipped": "turnstile-config-bypass"}

    secret = os.getenv("TURNSTILE_SECRET_KEY") or TURNSTILE_TEST_SECRET

    # When using Cloudflare's test secret, consider the challenge automatically passed.
    if secret == TURNSTILE_TEST_SECRET:
        return {"success": True, "skipped": "turnstile-dev-bypass"}
...
async def new_user(
    payload: user.register_user,
    req: Request,
    db: AsyncSession = Depends(database.get_db)):

    remote_ip = getattr(getattr(req, "client", None), "host", None)
    result = await verify_turnstile_token(payload.cf_token, remoteip=remote_ip)

    if not result.get("success"):
        detail = result if isinstance(result, dict) else {"detail": str(result)}
        raise HTTPException(
            status_code=400,
            detail={
    ...
    registeruser = User.User( 
        ...
        email=payload.email,
        password_hash=hashing.Hash.bcrypt(payload.password),
        api_key=api_key
    )
