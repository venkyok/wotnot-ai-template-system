  ...
  <main class="bg-container">
    <div class="w-full max-w-sm bg-white p-8 rounded-lg shadow-md">
      ...
      <form @submit.prevent="handleLogin" class="space-y-4">
        ...
        <button
          class="bg-gradient-to-r from-[#075e54] via-[#089678] to-[#075e54] text-white px-6 py-3 rounded-lg shadow-lg font-medium flex items-center justify-center hover:from-[#054d45] hover:via-[#067a62] hover:to-[#054d45] transition-all duration-300 disabled:opacity-60 disabled:cursor-not-allowed"
          type="submit"
          :disabled="isLoading">
...
export default {
  ...
  const baseUrl = this.apiUrl?.trim();
        ...
        const loginEndpoint = sanitizedBase
          ? `${sanitizedBase}/login`
          : "/api/login";
        ...
        if (rawPayload) {
          try {
            ...
            // Non-JSON response (likely HTML from a proxy or WAF)
            responseBody = null;
          }
        }

        if (!response.ok) {
          const fallbackMessage = rawPayload?.toLowerCase().includes("cloudflare")
  ...
  await this.$router.push({ name: "Broadcast2" });
      } catch (error) {
